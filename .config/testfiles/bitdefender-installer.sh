#!/bin/sh
# This script requires dpkg and apt to be installed (simulate debian environment)
# See https://bbs.archlinux.org/viewtopic.php?id=276989
# This script was hacked to work with arch cooperatively with grok:
# https://grok.com/c/37bf2093-575e-43da-9856-e85ae8af61c9?rid=80f0e9aa-8360-4f4e-8ebc-ddae7880f7c6
...
# variables populated by GravityZone
CONFIG_URL="https://cloudgz-ecs.gravityzone.bitdefender.com/Packages/BSTNIX/0/jfTnZI"
...
INSTALLER_DATE="Tue, 24 Mar 2026 15:25:50 +0200"
INSTALLER_VERSION="7.8.2.200288"
...
# space requirements
FILESCAN_DISK_SPACE_REMOTE=614400
FILESCAN_DISK_SPACE_FULL=1638400
FILESCAN_DISK_SPACE_LIGHT=1126400
FILESCAN_DISK_SPACE_REMOTE_FULL=1638400
FILESCAN_DISK_SPACE_REMOTE_LIGHT=1126400
UPDATESERVER_DISK_SPACE_LIMIT=10485760
PATCHMANAGEMENTSERVER_DISK_SPACE_LIMIT=104857600
...
# exit configured with 'trap <action> EXIT'
script_exit ()
{
	local err=$?
...
# Grok created this function; believes pkgfmt wasn't working when Arch got detected
update_globals ()
{
    local pkgfmt
    pkgfmt=deb
    ...
    printf "OS version: %s\n" "$OS_VERSION"
    printf "kernel: %s\n" "$(uname -r)"
    printf "Using package format: %s  →  %s\n" "$pkgfmt" "$PKG_FILE"
}
...
installed_version ()
{
	...
	case "$pkgmgr" in
		apt)
			# Handle for failure to purge with apt purge
...
get_proxy_config ()
{
	# If the proxy has already been configured, then exit
...
install_antimalware_plugins ()
{
	...
	# Bundled bdconfigure to speed up the exception processing
	local bdc="$DIR/bin/bdconfigure"

	# Delete update exceptions
	"$bdc" -s "/modules/update/scheduled/signatures/exceptions="

	# load the new update exceptions
...
control_feature ()
{
	...
	case "$feature" in
		...
		"UpdateServer" | "PatchManagementServer")
			...
			# update server settings to default
		...
		;;
		"IntegrityMonitor" | "ActivityMonitor")
			if [ "$enabled" = "true" ]; then
				if [ "$OS_BITNESS" = "32" ]; then
					printf "%s is not supported on 32-bit systems\n" "$module"
					enabled="false"
					...
					# Get the kernel version integer for testing
...
control_features ()
{
	...
	control_feature "ActivityMonitor" "activitymonitor"
}
...
check_disk_space ()
{
	printf "=== DISK SPACE CHECK BYPASSED ===\n"
	printf "Original required space logic skipped.\n"
	# Always succeed
	return 0
}
...
# omarchy/arch hack - Force debian and tooling if arch located, pkg*m* variables used to be detected
if [ -f /etc/arch-release ] || grep -q "arch" /etc/os-release 2>/dev/null; then
    printf "=== Arch Linux compatibility mode enabled ===\n"
    OS_ID="debian"
    OS_VERSION="12"
    pkgmgr=apt
    pkgfmt=deb
	...
	if [ -d "$productUpdateDownloadDir" ]; then
		# directory in format: /<somedir>/products
		local bn=$(basename "$productUpdateDownloadDir")
		if [ "$bn" = "products" ]; then
			printf "removing directory: %s ...\n" "$productUpdateDownloadDir"
			rm -rf "$productUpdateDownloadDir"
	...
	if [ -d "$signatureUpdateDownloadDir" ]; then
		# directory in format: /<somedir>/signatures
		local bn=$(basename "$signatureUpdateDownloadDir")
		if [ "$bn" = "signatures" ]; then
			printf "removing directory: %s ...\n" "$signatureUpdateDownloadDir"
			rm -rf "$signatureUpdateDownloadDir"
	...
	if [ -d "$kitsDownloadDir" ]; then
		# directory in format: /<somedir>/DownloadableKits
		local bn=$(basename "$kitsDownloadDir")
		if [ "$bn" = "DownloadableKits" ]; then
			printf "removing directory: %s ...\n" "$kitsDownloadDir"
			rm -rf "$kitsDownloadDir"
	...
	if [ -d "$patchDownloadDir" ]; then
		# directory in format: /<somedir>/patchManager
		local bn=$(basename "$patchDownloadDir")
		if [ "$bn" = "patchManager" ]; then
			printf "removing directory: %s ...\n" "$patchDownloadDir"
			rm -rf "$patchDownloadDir"
...
configure_submissions ()
{
	...
	if [ -n "$submit_nimbus" ]; then
		...
		if [ -f "$DIR/etc/bdnc.ini" ]; then
			if [ "$submit_nimbus" != "global" ] && [ "$submit_nimbus" != "<default>" ]; then
				sed -i "s#=.*nimbus.bitdefender.net#= $submit_nimbus.nimbus.bitdefender.net#" "$DIR/etc/bdnc.ini"
			else
				sed -i "s#=.*nimbus.bitdefender.net#= nimbus.bitdefender.net#" "$DIR/etc/bdnc.ini"
			fi
		fi
	fi
}
...
product_reconfigure ()
{
	...
	if [ "$use_ss" = "1" ]; then
		...
		if [ ! -f "$SETUP_DIR/update-server" ]; then
			...
			# contain any product update servers so we have to fallback to the installer config
...
product_repair ()
{
	...
	printf "configuring the product features\n"
		...
		NetworkMonitor IntegrityMonitor ContainerProtection ActivityMonitor; do
		local action
...
if [ $BDUID -ne 0 ]; then
	...
	LOGINPASSWD_DECODED=`printf "%s" "$LOGINPASSWD" | base64 -d`
	printf "%s\n" "$LOGINPASSWD_DECODED" | sudo -S FLOCKER="$FLOCKER" "$INSTALLER" "$@"
	die $?
fi
...
# Signature verification after having fetched
