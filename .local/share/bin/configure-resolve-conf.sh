...
if [ "$RC" -eq 0 ]; then
  mv "${CONF}.tmp" "$CONF" || { printf "Failed to update config.\n" >&2; rm -f "${CONF}.tmp"; exit 1; }
  printf "Updated %s\n" "$CONF"
  exit 0
fi
...
if [ "$RC" -eq 2 ]; then
  if grep -q '^\s*\[Resolve\]' "$CONF"; then
    awk -v dom="$DOMAIN" '
      ...
      { print }
    ' "$CONF" > "${CONF}.tmp" && mv "${CONF}.tmp" "$CONF" && printf "Inserted Domains= under [Resolve] in %s\n" "$CONF" && exit 0
    ...
    # Append new [Resolve] section
    {
      printf "\n[Resolve]\n"
      printf "Domains=%s\n" "$DOMAIN"
    } >> "$CONF" || { printf "Failed to append [Resolve] section.\n" >&2; exit 1; }
